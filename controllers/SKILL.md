...
- стабильный API-контракт и предсказуемое versioning-поведение;
...
7. Любой breaking change в опубликованной версии API выполнять только через новую версию.
...
1. Использовать ресурсный стиль маршрутов в новых версиях API.
...
9. Располагать методы контроллера в логическом порядке: сначала GET-методы (чтение), затем методы изменения данных, сгруппированные по полю/ресурсу (Set и Init для одного поля — рядом).

## Анти-паттерны
1. Бизнес-правила и вычисления в контроллере.
2. Breaking-изменения в `v1` после публикации.
...
3. Для breaking change создай новую версию контроллера и оставь старую версию неизменной.
...
7. Проверь совместимость старых клиентов.
...
7. Breaking-изменения вынесены в новую версию API.
